...
@onready var goal_label: Label = $TopPanel/Content/GoalLabel
...
func set_phase(phase_number: int, phase_data: Dictionary, phase_seed: int) -> void:
	...
	var safe_cell: Array = phase_data.get("safe_spot", [])
	goal_label.visible = safe_cell.size() == 2
	if goal_label.visible:
		goal_label.text = "Destino  •  coluna %d  •  linha %d" % [int(safe_cell[0]) + 1, int(safe_cell[1]) + 1]
	status_label.text = "WASD para mover  •  R para reiniciar a fase."
	_reset_edito_display()
